...
- Macro catalog expanded to **1292 curated `e_*` columns** across **44 economies** (US, Mexico, euro area + ~40 DM/EM countries) and ~59 distinct indicators, up from 17. Cross-region consistency comes from wide DBnomics datasets where one code pattern spans many countries, swapping the country code per concept:
  - **Rates & money:** central-bank policy rates (BIS `WS_CBPOL`), short rates / 10Y govt yields (IMF `IFS` / Eurostat), narrow M1 & broad M3 money (OECD `MEI`), credit to the private sector (BIS `WS_TC`), REER & NEER (BIS `WS_EER`).
  - **Prices:** CPI, core CPI, PPI (IMF `IFS`), HICP (Eurostat), house prices (BIS `WS_SPP`).
  - **Activity & sentiment:** real/nominal GDP & GDP growth (World Bank `WDI` / Eurostat), industrial production, retail sales (Eurostat), PMI proxy / business & consumer confidence (OECD), economic sentiment (Eurostat ESI).
  - **Labour:** unemployment rate, harmonised unemployment, youth & long-term unemployment (IMF / Eurostat / World Bank).
  - **External:** FX vs USD & reserves (IMF `IFS`), current account, goods & services exports/imports, goods trade balance (World Bank / IMF `DOT`), government debt & budget balance %GDP (IMF `WEO`).
  - Plus deeper US coverage via FRED (PPI, PCE, 3M/5Y/30Y, IP, retail, payrolls, trade balance, SOFR, term spread, …).

  Every series id was machine-verified against the live provider API before inclusion (`scripts/build_macro_catalog.py`) — hallucination-free; ~290 non-existent candidates were dropped. `commercial_ok` is flagged per underlying source (Eurostat / World Bank = `yes`; IMF / BIS / OECD = `restricted`; FRED = `no`, conservative).
- The panel column picker now **nests the macro columns under a single `Economic (macro)` set, with one collapsible subgroup per country/area** (44 of them) inside it — a two-level tree (Economic → country → columns) generated from the catalog, so the picker stays navigable at this scale and new columns appear automatically.
...
- Standalone (ticker-free) macro export: a run with no identifiers but ≥1 `e_*` column writes each selected series as its own `(m_date, value)` table at native cadence (no forward-fill), through every configured output handler — CSV/Parquet write one `{column}.{ext}` file per series, DuckDB upserts the series into the shared `curated_data` table keyed on `(main_identifier, m_date)`, and the in-memory handler indexes it by date. The series id is the output's `main_identifier`. (The date column is `m_date`, matching every other output, so DuckDB/in-memory no longer reject a macro-only run.)
