...
- The panel column picker now **groups the macro columns by country/area** — one collapsible `Economic · <Country> (<REGION>)` section per economy (44 of them) generated from the catalog, so the picker stays navigable at this scale and new columns appear automatically.
